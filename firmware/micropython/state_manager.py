"""Pet state machine for copilot-buddy (MicroPython).
...
Identical logic to the CircuitPython version — the state machine is
pure Python with no hardware dependencies.  The caller passes ``now``
(milliseconds as an int) from ``time.ticks_ms()``.
"""
...
# Hold durations for timed states (milliseconds)
HOLD_ATTENTION = 3000
HOLD_CELEBRATE = 5000
HOLD_DIZZY = 3000
HOLD_HEART = 2000

# Disconnect thresholds (milliseconds)
DISCONNECT_WARN = 10000
DISCONNECT_SLEEP = 300000
...
class StateManager:
    ...
    def process_message(self, msg, now):
        ...
        *msg* may be a heartbeat (has ``state`` key) or event
        (has ``evt`` key).  *now* is milliseconds (int) from
        ``time.ticks_ms()``.
    ...
    def _handle_event(self, msg, now):
        ...
        if evt == "start":
            ...
            duration = time.ticks_diff(now, self._last_start_time)
            if duration < 3000:
    ...
    def _resolve(self, now):
        ...
        if self._timed_state is not None:
            ...
            self.state = (
                self._timed_state if ts_pri >= bs_pri else self.base_state
            )
        else:
